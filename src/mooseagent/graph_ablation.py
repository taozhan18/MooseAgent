...
def modify(state: FlowState, config: RunnableConfig):
    ...
    helper = load_chat_model(configuration.alignment_model)  # .with_structured_output(ExtracterFileState)
    helper_answer = helper.invoke([HumanMessage(content=messages[0]["content"])])
    feedback = helper_answer.content
...
if __name__ == "__main__":
    ...
    topic = """
Steady-state thermodynamic simulations are performed for 16 prismatic fuel elements, using the assumption of plane strain. The mesh has been generated; please load the file represent_mesh.e. It includes 16 prismatic fuel elements. The computational domain consists only of the matrix. Its Young’s modulus is temperature-dependent: 211600*1E6. The Poisson’s ratio is 0.21. The thermal conductivity is given by 9.2. The thermal expansion coefficient is defined as: 5e-6.
For boundary conditions, the mesh file has defined the following boundary names: bottom, left, right, heatpipe, and matrixin_b. Set a constant temperature boundary condition of 948 °C on heatpipe. Apply Neumann temperature boundary conditions with a value of 100000.0 on matrixin_b. Also, set a pressure boundary condition of 2e6 on matrixin_b. Impose no-displacement boundary conditions on bottom and left, both disp_x and disp_y.
