...
from mooseagent.graph_ablation import architect_builder, MemorySaver
...
async def run_experiment(topic: str, n_runs: int = 5, folder_name="unknown"):
    stats = ExperimentStats()
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")

    # 创建实验目录
    experiment_dir = os.path.join(run_path, f"experiments/{folder_name}_{timestamp}")
    ...
    subprocess.run(["rm", "-r", os.path.join(configuration.save_dir)])
    for i in range(n_runs):
        new_save_dir = os.path.join(experiment_dir, f"run_{i+1}")
        os.makedirs(configuration.save_dir, exist_ok=True)
        # 为每次运行创建单独的日志文件
        run_log = os.path.join(configuration.save_dir, f"run_{i+1}.log")
        ...
        try:
            with get_openai_callback() as cb:
                ...
                subprocess.run(["mv", configuration.save_dir, new_save_dir])
                # 收集统计数据
                stats.add_run(
...
if __name__ == "__main__":
    for i in range(21, 22):
        with open(os.path.join(run_path, "database/cases", f"case{i+1}.txt"), "r") as f:
            ...
            print("current topic: \n", topic)
            asyncio.run(run_experiment(topic, n_runs=5, folder_name=f"case{i+1}"))
